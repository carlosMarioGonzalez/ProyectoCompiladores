...
#Contiene todos los diccionarios
aux = {}                     
aux.update(di.subject)
aux.update(di.auxiliarneg)
...
def p_textoSPnot(p):
    'texto : frase_presenteSimpleNeg texto'
    p[0] = p[1] + " " + p[2]
...
def p_textoPastfuturo(p):
    'texto : frase_futurosimple texto'
    p[0] = p[1] + " " + p[2]
...
#FRASE COMPUESTA
def p_Frase_SP_compuesta(p):
    'frase_presenteSimple : SUBJECT VERB_Spresent predicated frase_presenteSimple'
    p[0] = p[1] + " " + p[2] + p[3] + p[4]


def p_Frase_SPNeg(p):
    'frase_presenteSimpleNeg : SUBJECT AUXNEG VERB_Spresent predicated'
    p[0] = p[1] + " " + p[2] + " " + p[3] + p[4]

def p_Frase_SPNeg3(p):
    'frase_presenteSimpleNeg : SUBJECT AUXNEG3 VERB_Spresent predicated'
    p[0] = p[1] + " " + p[2] + " " + p[3] + p[4]
...
#--------------------
def p_Frase_futuro(p):
    'frase_futurosimple : SUBJECT VERB_FUTURO predicated'
    p[0] = p[1] + " " + p[2]+ " " + p[3]
